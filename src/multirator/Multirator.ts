...
export class Multirator {
  ...
  public add(...rules: MultiratorRule[]): void {
  ...
  protected validateRule(rule: MultiratorRule): void {
    if (!rule.id) {
      throw new Error(
        `Replacement rule (${rule.match}) should have a defined id`,
      );
    }

    if (this.ruleIDs.has(rule.id)) {
      throw new Error(
        `Replacement rule (${rule.match}) has a non-unique ID = ${rule.id}`,
      );
    }
  }
  ...
  public process(record: BareRecord): ProcessedLemma[] {
    ...
    for (const option of record.isv.lemmas()) {
    ...
    for (const rule of this.rules) {
      ...
      for (const sourceString of currentReplacements) {
        for (const replacement of rule.replaceWith as any[]) {
  ...
  public getDifference(
    flavorizedLemmas: ProcessedLemma[],
    targetSynset?: core.Synset | null,
  ): number {
    let minimalDistance = Number.POSITIVE_INFINITY;

    const targetOptions = targetSynset ? [...targetSynset.lemmas()] : [];
